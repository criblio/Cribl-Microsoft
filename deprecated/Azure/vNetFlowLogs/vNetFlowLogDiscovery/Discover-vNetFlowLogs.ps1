...
function Ensure-AzureConnection {
 ...
 if (-not $context) {
 Write-Host " ℹ No active Azure session found." -ForegroundColor Yellow
 ...
 Write-Host " Connected to wrong tenant!" -ForegroundColor Yellow
 ...
 Write-Host " Azure session expired or invalid." -ForegroundColor Yellow
...
function Find-vNetFlowLogStorageAccounts {
 ...
 Write-Host " ℹ No storage accounts found in this subscription" -ForegroundColor DarkGray
 continue
 }
 ...
 Write-Host " ⏭ No Flow Logs" -ForegroundColor DarkGray
 }
 }
 catch {
 Write-Host " Error checking: $($_.Exception.Message)" -ForegroundColor Yellow
 }
 }
 }
...
if ($discoveredAccounts.Count -eq 0) {
 Write-Host "`n No storage accounts with vNet Flow Logs were found." -ForegroundColor Yellow
...
Write-Host "`n IMPORTANT - REQUIRED PERMISSIONS" -ForegroundColor Yellow
