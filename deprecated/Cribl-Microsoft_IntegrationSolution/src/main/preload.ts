...
contextBridge.exposeInMainWorld('api', {
  ...
  sentinelRepo: {
    ...
    resetError: (): Promise<unknown> =>
      ipcRenderer.invoke('sentinel-repo:reset-error'),
    ...
    blocklistAdd: (solutionName: string, reason: string): Promise<{ added: boolean; blocklist: Array<{ name: string; reason: string; source: string }> }> =>
      ipcRenderer.invoke('sentinel-repo:blocklist-add', { solutionName, reason }),
    ...
    onFetchProgress: (callback: (data: { done: number; total: number; pct: number }) => void) => {
      const handler = (_: unknown, data: { done: number; total: number; pct: number }) => callback(data);
      ipcRenderer.on('sentinel-repo:fetch-progress', handler);
      return () => ipcRenderer.removeListener('sentinel-repo:fetch-progress', handler);
    },
  },
  packBuilder: {
  ...
  auth: {
    ...
    criblReconnect: (overrides?: { deploymentType?: string; baseUrl?: string; organizationId?: string; clientId?: string }): Promise<{ success: boolean; error?: string }> =>
      ipcRenderer.invoke('auth:cribl-reconnect', overrides),
    criblSaved: (): Promise<{
    ...
    githubSaved: (): Promise<{ hasPat: boolean }> =>
      ipcRenderer.invoke('auth:github-saved'),
    githubSave: (pat: string): Promise<{ success: boolean; login?: string; error?: string }> =>
      ipcRenderer.invoke('auth:github-save', { pat }),
    ...
    azureCreateWorkspace: (name: string, resourceGroup: string, location: string, subscriptionId?: string): Promise<{
      success: boolean; customerId?: string; error?: string;
    }> => ipcRenderer.invoke('auth:azure-create-workspace', { name, resourceGroup, location, subscriptionId }),
    azureEnableSentinel: (workspaceName: string, resourceGroup: string, subscriptionId?: string): Promise<{
      success: boolean; alreadyEnabled?: boolean; error?: string;
    }> => ipcRenderer.invoke('auth:azure-enable-sentinel', { workspaceName, resourceGroup, subscriptionId }),
  ...
  sampleParser: {
    ...
    parseCsvWithHeaders: (csvContent: string, headers: string[], skipFirstRow: boolean): Promise<{
      ...
      fields: Array<{ name: string; type: string; sampleValues: string[]; occurrence: number; required: boolean }>;
      rawEvents: string[]; sourceName: string; timestampField: string; errors: string[];
    }> => ipcRenderer.invoke('samples:parse-csv-with-headers', { csvContent, headers, skipFirstRow }),
  ...
  elasticRepo: {
    ...
    resetError: (): Promise<unknown> =>
      ipcRenderer.invoke('elastic-repo:reset-error'),
    ...
    onFetchProgress: (callback: (data: { done: number; total: number; pct: number }) => void) => {
      const handler = (_: unknown, data: { done: number; total: number; pct: number }) => callback(data);
      ipcRenderer.on('elastic-repo:fetch-progress', handler);
      return () => ipcRenderer.removeListener('elastic-repo:fetch-progress', handler);
    },
  },
