...
describe('matchSampleToTable', () => {
  it('returns the default table when there are 0 or 1 destination tables', () => {
    expect(matchSampleToTable('anything', [], 1, 'CommonSecurityLog')).toBe('CommonSecurityLog');
    expect(matchSampleToTable('anything', [{ id: 'x', destTable: 'T' }], 0, 'Def')).toBe('Def');
  });
  ...
  it('matches a sample log type to a vendor log type via normalized id/name', () => {
    ...
    expect(matchSampleToTable('FDR_Events', lts, 2, 'Def')).toBe('ASimNetworkSessionLogs');
  });
