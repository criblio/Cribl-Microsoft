...
function SentinelIntegration() {
    ...
    const init = async () => {
      try {
        ...
        if (mode === 'air-gapped') {
          ...
          // Load auth status in the background -- don't block solution loading.
          // Cribl reconnect can timeout (30s+) if the leader is unreachable.
          (async () => {
            try {
                ...
                if (auth.azure.subscriptionId) {
                  ...
                  try {
                    const [wsResult, rgResult] = await Promise.all([
                      window.api.auth.azureWorkspaces(auth.azure.subscriptionId),
                      window.api.auth.azureResourceGroups(auth.azure.subscriptionId),
                    ]);
        ...
        // Load solutions immediately (don't wait for auth to complete)
        try {
          ...
          if (repoStatus.state === 'ready') {
            ...
            // Don't auto-clone here -- the Repos page handles cloning.
            // Just show the current state so the user knows to go to Repos.
            setRepoState(repoStatus.state as typeof repoState);
          }
  ...
  useEffect(() => {
    if (state.selectedSolution) {
      // Build a short pack name: strip noise words, take first 2 meaningful words, add -sentinel
      const name = state.selectedSolution.toLowerCase()
  ...
  // Rename a sample's log type (affects pipeline names, sample filenames, routes)
  function renameSampleLogType(oldName: string, newName: string) {
    const trimmed = newName.trim().replace(/[^a-zA-Z0-9_\- ]/g, '');
    if (!trimmed || trimmed === oldName) {
      setEditingLogType(null);
      return;
    }
    // Update all samples with the old log type
    const updated = state.samples.map((s: any) =>
      s.logType === oldName ? { ...s, logType: trimmed } : s
    );
    update({ samples: updated });
    // Update expanded panel if it was showing the renamed sample
    if (expandedSample === oldName) setExpandedSample(trimmed);
    setEditingLogType(null);
  }

  function detectLogType(r: { sourceName: string; fields?: Array<{ name: string; sampleValues?: string[] }> }): string {
    ...
    if (r.fields) {
      ...
      if (stField?.sampleValues?.[0]) {
        const parts = stField.sampleValues[0].split(':');
        const lt = parts.length > 1 ? parts[parts.length - 1].replace(/[^a-zA-Z]/g, '') : parts[0];
        return lt.charAt(0).toUpperCase() + lt.slice(1);
      }
    }
    return r.sourceName.replace(/\.[^.]+$/, '').replace(/[^a-zA-Z0-9]/g, '_');
  }

  // Check if parsed fields are headerless CSV (_0, _1, _2 pattern)
  function isHeaderlessCsv(fields: Array<{ name: string }>): boolean {
    ...
    try {
      ...
      if (results.length > 0 && state.selectedSolution) {
        for (const r of results) {
          const lt = pasteLogType || detectLogType(r);

          // Check for headerless CSV -- if detected, show the header dialog
          if (r.format === 'csv' && isHeaderlessCsv(r.fields)) {
            // Reconstruct CSV content from rawEvents (they're JSON-stringified objects)
            const csvLines: string[] = [];
            for (const raw of r.rawEvents) {
              try {
            ...
            setCsvHeaderDialog({
              ...
              firstRows: csvLines.slice(0, 3),
            });
            setCsvParsedHeaders([]);
            setCsvFeedConfigText('');
            setCsvFeedVendor('');
            setCsvHeaderTab('file');
            return; // pause upload flow -- dialog will continue on "Apply"
          }
  ...
  const handleApplyCsvHeaders = async () => {
    if (!window.api || !csvHeaderDialog || csvParsedHeaders.length === 0 || !state.selectedSolution) return;
    try {
      const result = await window.api.sampleParser.parseCsvWithHeaders(
        csvHeaderDialog.csvContent,
        csvParsedHeaders,
        false, // don't skip first row (it's data)
      );
      if (result.eventCount > 0) {
        // Tag the re-parsed sample
        await window.api.sampleParser.tagSample(
          state.selectedSolution,
          csvHeaderDialog.logType,
          result.rawEvents.join('\n'),
          csvHeaderDialog.sourceName,
        );
        const tagged = await window.api.sampleParser.getTagged(state.selectedSolution);
        update({ samples: tagged });
      }
  ...
  // Upload a header file (column names, one per line or comma-separated)
  const handleUploadHeaderFile = async () => {
    if (!window.api) return;
    try {
      const results = await window.api.sampleParser.parseFiles();
      if (results.length > 0) {
        const content = results[0].rawEvents.join('\n');
        // Try comma-separated first, then newline-separated
        let headers: string[];
        if (content.includes(',')) {
          headers = content.split(',').map((h) => h.trim().replace(/^["']|["']$/g, '')).filter(Boolean);
        } else {
          headers = content.split('\n').map((h) => h.trim().replace(/^["']|["']$/g, '')).filter(Boolean);
        }
        // Clean: only keep valid identifier-like names
        headers = headers.map((h) => h.replace(/[^a-zA-Z0-9_]/g, '_').replace(/^_+/, '')).filter(Boolean);
        if (headers.length > 0) {
          setCsvParsedHeaders(headers);
  ...
  const handleParseFeedConfig = async () => {
    if (!window.api || !csvFeedConfigText.trim()) return;
    try {
      const result = await window.api.sampleParser.parseFeedConfig(csvFeedConfigText);
      if (result.fields.length > 0) {
        setCsvParsedHeaders(result.fields);
        setCsvFeedVendor(`${result.vendor} ${result.feedType} (${result.fields.length} fields)`);
      } else {
  ...
  // Sample browse/select modal state
  const [browseOpen, setBrowseOpen] = useState(false);
  const [browseLoading, setBrowseLoading] = useState(false);
  const [browseItems, setBrowseItems] = useState<Array<{
    id: string; tier: string; source: string; logType: string;
    format: string; eventCount: number; fileName: string; preview?: string[];
  }>>([]);
  const [browseSelected, setBrowseSelected] = useState<Set<string>>(new Set());
  const [browseCommitting, setBrowseCommitting] = useState(false);
  ...
  const [editLogTypeValue, setEditLogTypeValue] = useState('');

  // CSV header dialog state -- shown when headerless CSV is detected
  const [csvHeaderDialog, setCsvHeaderDialog] = useState<{
    open: boolean;
    csvContent: string;    // raw CSV content to re-parse
    columnCount: number;   // number of columns detected
    logType: string;       // log type name for the sample
    sourceName: string;    // filename
    firstRows: string[];   // first 3 data rows for preview
  } | null>(null);
  const [csvHeaderTab, setCsvHeaderTab] = useState<'file' | 'config'>('file');
  const [csvFeedConfigText, setCsvFeedConfigText] = useState('');
  const [csvParsedHeaders, setCsvParsedHeaders] = useState<string[]>([]);
  const [csvFeedVendor, setCsvFeedVendor] = useState('');
  ...
  // Browse Samples: open modal and fetch available samples from all tiers
  const handleBrowseSamples = async () => {
    ...
    try {
      const items = await (window.api as any).sampleResolver.listAvailable(state.selectedSolution);
      setBrowseItems(items || []);
      // Pre-select sentinel-repo and elastic samples by default
      const preSelected = new Set<string>();
      for (const item of (items || [])) {
        if (item.tier === 'sentinel-repo' || item.tier === 'elastic') {
  ...
  // Commit selected samples from the browse modal
  const handleCommitBrowseSelection = async () => {
    if (!window.api || !state.selectedSolution || browseSelected.size === 0) return;
    setBrowseCommitting(true);
    setPreIngestedWarning('');
    try {
      const selectedIds = [...browseSelected];
      const loaded = await (window.api as any).sampleResolver.loadSelected(state.selectedSolution, selectedIds);

      const formats: Record<string, string> = { ...originalSampleFormats };
      const tierCounts: Record<string, number> = {};

      for (const sample of (loaded || [])) {
        const logType = sample.logType || sample.tableName || 'default';
        const tier = sample.tier || 'unknown';
        tierCounts[tier] = (tierCounts[tier] || 0) + 1;

        if (sample.format && sample.format !== 'ndjson' && sample.format !== 'json') {
          formats[logType.toLowerCase()] = sample.format;
        }
        // Events are now proper JSON with named fields (PAN-OS CSV converted,
        // syslog parsed, etc.) so tagSample can parse them correctly.
        const content = (sample.rawEvents || []).join('\n');
        if (content.trim()) {
          await window.api.sampleParser.tagSample(
            state.selectedSolution, logType, content, sample.source || tier,
          );
        }
      }
      ...
      const parts: string[] = [];
      if (tierCounts['sentinel-repo']) parts.push(`${tierCounts['sentinel-repo']} from Sentinel repo`);
      if (tierCounts['elastic']) parts.push(`${tierCounts['elastic']} from Elastic integrations`);
      if (parts.length > 0) {
        setPreIngestedWarning(`Loaded ${parts.join(', ')}.`);
      }
  ...
  const handleDeploy = async () => {
    ...
    try {
      ...
      // Samples are the primary source of log types (user selected/renamed these).
      // Generate route filters from sample data so each pipeline only processes its log type.
      if (state.samples.length > 0) {
        let routeField = '';
        const logTypeValues = new Map<string, string>();

        if (state.samples.length > 1) {
          // Strategy 1: Find a field in the event data that's unique per log type
          const discriminatorFields = [
            'sourcetype', 'type', 'subtype', 'log_type', 'logType',
            'category', 'event_type', 'eventType', 'dataset', 'action',
            'DeviceEventClassID', 'Activity', 'module',
          ];
          for (const field of discriminatorFields) {
            const fieldValues = new Map<string, string>();
            let found = true;
            for (const sample of state.samples) {
              if (!sample.rawEvents?.length) { found = false; break; }
              try {
                const evt = JSON.parse(sample.rawEvents[0]);
            ...
            if (found && fieldValues.size === state.samples.length) {
              ...
              if (uniqueValues.size === state.samples.length) {
                routeField = field;
                for (const [lt, val] of fieldValues) logTypeValues.set(lt, val);
                break;
              }
            }
          }

          // Strategy 2: If no unique event field found, check sample field names
          // for a field that has the log type name as its value
          if (!routeField) {
            for (const field of discriminatorFields) {
              const fieldValues = new Map<string, string>();
              let partialMatch = true;
              for (const sample of state.samples) {
                if (!sample.rawEvents?.length) { partialMatch = false; break; }
                try {
                  const evt = JSON.parse(sample.rawEvents[0]);
          ...
          // Strategy 3: If still nothing, use sourcetype pattern based on log type name
          // e.g., sourcetype.match(/web/i) for WEB, sourcetype.match(/fw|firewall/i) for firewall
          if (!routeField) {
            routeField = '__logType';
            for (const sample of state.samples) {
              logTypeValues.set(sample.logType, sample.logType.toLowerCase());
            }
          }
        }
        ...
        for (const sample of state.samples) {
          ...
          if (!seenLogTypes.has(sampleKey)) {
            ...
            let sourcetypeFilter = 'true';
            if (state.samples.length > 1 && logTypeValues.has(sample.logType)) {
              const val = logTypeValues.get(sample.logType)!;
              if (routeField === '__logType') {
                // Use sourcetype regex match as last resort
                sourcetypeFilter = `sourcetype && sourcetype.match(/${val}/i)`;
              } else {
                sourcetypeFilter = `${routeField}=='${val}'`;
              }
            }
            tables.push({
        ...
        if (routeField && routeField !== '__logType') {
          addLog(`  Route discriminator: ${routeField} (${[...logTypeValues.entries()].map(([k, v]) => `${k}=${v}`).join(', ')})`);
        } else if (routeField === '__logType') {
          addLog(`  Route filter: sourcetype regex match (no unique discriminator field found)`);
        }
      }
      ...
      console.log('[deploy] tables:', JSON.stringify(tables.map((t: any) => ({ logType: t.logType, sentinelTable: t.sentinelTable }))));
  ...
  return (
    <div style={s.page}>
      ...
      <div style={{
        ...
        <span style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
          ...
          <span style={{ color: elasticRepoState.state === 'ready' ? 'var(--accent-green)' : 'var(--text-muted)' }}>
            Elastic {elasticRepoState.state === 'ready' ? `(${elasticRepoState.packageCount} samples)` : elasticRepoState.state}
          </span>
        </span>
      </div>
          ...
          <div style={{ marginBottom: '12px' }}>
            <div style={{ marginBottom: '8px' }}>
              {state.samples.map((sample) => (
                ...
                >
                  {editingLogType === sample.logType ? (
                    <input
                      style={{
                        fontWeight: 700, fontSize: '12px', padding: '1px 4px', width: '140px',
                        background: 'var(--bg-primary)', border: '1px solid var(--accent-blue)',
                        borderRadius: '3px', color: 'var(--text-primary)', fontFamily: 'inherit',
                      }}
                      value={editLogTypeValue}
                      onChange={(e) => setEditLogTypeValue(e.target.value)}
                      onBlur={() => renameSampleLogType(sample.logType, editLogTypeValue)}
                      onKeyDown={(e) => {
                        if (e.key === 'Enter') renameSampleLogType(sample.logType, editLogTypeValue);
                        if (e.key === 'Escape') setEditingLogType(null);
                      }}
                      autoFocus
                    ...
                    <>
                      ...
                      <span
                        style={{
                          cursor: 'pointer', marginLeft: '6px', fontSize: '10px',
                          color: 'var(--accent-blue)', textDecoration: 'underline',
                        }}
                        title="Rename log type (affects pipeline and route names)"
                        onClick={(e) => {
                          e.stopPropagation();
                          setEditingLogType(sample.logType);
                          setEditLogTypeValue(sample.logType);
                        }}
                      >rename</span>
                    </>
                  )}
        ...
        <div style={s.row}>
          ...
          <div style={s.field}>
            ...
            <div style={{ display: 'flex', gap: '6px', marginTop: '8px', flexWrap: 'wrap' }}>
              ...
              <button className="btn-success" style={{ fontSize: '11px' }} onClick={handleBrowseSamples}
                disabled={!state.selectedSolution || browseOpen}>
                Browse Samples
              </button>
            </div>
          </div>
      ...
      {/* CSV Header Dialog -- shown when headerless CSV is detected */}
      {csvHeaderDialog?.open && (
        <div style={{
          ...
          <div style={{
            background: 'var(--bg-secondary)', border: '1px solid var(--border-color)',
            borderRadius: '8px', padding: '24px', width: '680px', maxHeight: '80vh',
            display: 'flex', flexDirection: 'column', boxShadow: '0 8px 32px rgba(0,0,0,0.4)',
          }}>
            <div style={{ fontSize: '16px', fontWeight: 700, marginBottom: '4px' }}>
              Headerless CSV Detected ({csvHeaderDialog.columnCount} columns)
            </div>
            <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginBottom: '16px' }}>
              The uploaded CSV has no header row. Provide field names so the app can map them to the DCR schema.
            </div>

            {/* Tabs */}
            <div style={{ display: 'flex', gap: '0', marginBottom: '12px', borderBottom: '1px solid var(--border-color)' }}>
              {(['file', 'config'] as const).map((tab) => (
                <button key={tab}
                  style={{
                    padding: '6px 16px', fontSize: '12px', fontWeight: csvHeaderTab === tab ? 700 : 400,
                    background: 'transparent', border: 'none', cursor: 'pointer',
                    color: csvHeaderTab === tab ? 'var(--accent-blue)' : 'var(--text-muted)',
                    borderBottom: csvHeaderTab === tab ? '2px solid var(--accent-blue)' : '2px solid transparent',
                  }}
                  onClick={() => setCsvHeaderTab(tab)}>
                  {tab === 'file' ? 'Upload Header File' : 'Paste Feed Config'}
                </button>
              ))}
            </div>

            {/* Tab 1: Upload header file */}
            {csvHeaderTab === 'file' && (
              <div style={{ marginBottom: '12px' }}>
                <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginBottom: '8px' }}>
                  Upload a text file with ordered column names -- one per line or comma-separated.
                </div>
                <button className="btn-secondary" style={{ fontSize: '11px' }} onClick={handleUploadHeaderFile}>
                  Choose Header File
                </button>
              </div>
            )}

            {/* Tab 2: Paste feed config */}
            {csvHeaderTab === 'config' && (
              <div style={{ marginBottom: '12px' }}>
                <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginBottom: '8px' }}>
                  Paste your vendor's output feed configuration (Zscaler NSS format string, PAN-OS syslog profile, etc.)
                </div>
                <textarea
                  style={{
                    width: '100%', height: '80px', padding: '8px', fontSize: '11px',
                    background: 'var(--bg-input)', border: '1px solid var(--border-color)',
                    borderRadius: '4px', color: 'var(--text-primary)', fontFamily: 'var(--font-mono)',
                    resize: 'vertical',
                  }}
                  value={csvFeedConfigText}
                  onChange={(e) => setCsvFeedConfigText(e.target.value)}
                  placeholder={'Example Zscaler NSS format:\n%s{datetime},%s{cloudname},%s{host},%s{serverip},%d{action},...\n\nOr field list:\ndatetime,cloudname,host,serverip,action,...'}
                />
                <button className="btn-primary" style={{ fontSize: '11px', marginTop: '6px' }}
                  onClick={handleParseFeedConfig}
                  disabled={!csvFeedConfigText.trim()}>
                  Parse Config
                </button>
              </div>
            )}
            ...
            {csvFeedVendor && (
              <div style={{
                padding: '8px 12px', borderRadius: '4px', fontSize: '11px', marginBottom: '12px',
                background: csvParsedHeaders.length > 0 ? 'rgba(102, 187, 106, 0.08)' : 'rgba(255, 167, 38, 0.08)',
                border: `1px solid ${csvParsedHeaders.length > 0 ? 'rgba(102, 187, 106, 0.2)' : 'rgba(255, 167, 38, 0.2)'}`,
                color: csvParsedHeaders.length > 0 ? 'var(--accent-green)' : 'var(--accent-orange)',
              }}>
                {csvFeedVendor}
                {csvParsedHeaders.length > 0 && csvParsedHeaders.length !== csvHeaderDialog.columnCount && (
                  <span style={{ color: 'var(--accent-orange)', marginLeft: '8px' }}>
                    (header count {csvParsedHeaders.length} differs from CSV columns {csvHeaderDialog.columnCount})
                  </span>
                )}
              </div>
            )}

            {/* Preview with headers applied */}
            {csvParsedHeaders.length > 0 && csvHeaderDialog.firstRows.length > 0 && (
              <div style={{ marginBottom: '12px' }}>
                <div style={{ fontSize: '11px', fontWeight: 600, color: 'var(--text-secondary)', marginBottom: '4px' }}>
                  Preview (first row with headers applied)
                </div>
                <div style={{
                  padding: '8px', background: 'var(--bg-primary)', borderRadius: '4px',
                  border: '1px solid var(--border-color)', fontFamily: 'var(--font-mono)',
                  fontSize: '10px', maxHeight: '120px', overflow: 'auto',
                }}>
                  {(() => {
                    const values = csvHeaderDialog.firstRows[0].split(',').map((v) => v.trim().replace(/^"|"$/g, ''));
                    return csvParsedHeaders.slice(0, 15).map((h, i) => (
                      <div key={i} style={{ display: 'flex', gap: '8px', padding: '1px 0' }}>
                        <span style={{ color: 'var(--accent-blue)', fontWeight: 600, minWidth: '140px' }}>{h}</span>
                        <span style={{ color: 'var(--text-secondary)' }}>{values[i] !== undefined ? values[i] : '(no value)'}</span>
                      </div>
                    ));
                  })()}
                  {csvParsedHeaders.length > 15 && (
                    <div style={{ color: 'var(--text-muted)', padding: '2px 0' }}>...and {csvParsedHeaders.length - 15} more fields</div>
                  )}
                </div>
              </div>
            )}
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '12px', borderTop: '1px solid var(--border-color)' }}>
              <div style={{ fontSize: '11px', color: 'var(--text-muted)' }}>
                {csvParsedHeaders.length > 0
                  ? `${csvParsedHeaders.length} headers ready to apply`
                  : 'Upload a header file or paste a feed config above'}
              </div>
              <div style={{ display: 'flex', gap: '8px' }}>
                <button className="btn-secondary" style={{ fontSize: '12px', padding: '6px 16px' }}
                  onClick={() => {
                    // Skip headers -- load with _0, _1, _2
                    setCsvHeaderDialog(null);
                  }}>
                  Skip
                </button>
                <button className="btn-primary" style={{ fontSize: '12px', padding: '6px 16px' }}
                  onClick={handleApplyCsvHeaders}
                  disabled={csvParsedHeaders.length === 0}>
                  Apply Headers
                </button>
              </div>
            </div>
      ...
      {/* Sample Browse/Select Modal */}
      {browseOpen && (
        <div style={{
          position: 'fixed', top: 0, left: 0, right: 0, bottom: 0,
          background: 'rgba(0,0,0,0.6)', zIndex: 1000,
          display: 'flex', alignItems: 'center', justifyContent: 'center',
        }} onClick={() => !browseCommitting && setBrowseOpen(false)}>
          <div style={{
            background: 'var(--bg-secondary)', border: '1px solid var(--border-color)',
            borderRadius: '8px', padding: '24px', width: '700px', maxHeight: '80vh',
            display: 'flex', flexDirection: 'column', boxShadow: '0 8px 32px rgba(0,0,0,0.4)',
          }} onClick={(e) => e.stopPropagation()}>
            <div style={{ fontSize: '16px', fontWeight: 700, marginBottom: '4px' }}>
              Browse Sample Sources
            </div>
            <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginBottom: '16px' }}>
              Select samples to load for {state.selectedSolution}. Sources are searched automatically based on the selected solution.
            </div>

            {browseLoading ? (
              <div style={{ textAlign: 'center', padding: '40px 0', color: 'var(--text-muted)', fontSize: '13px' }}>
                Searching Elastic integrations...
              </div>
            ) : browseItems.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '40px 0', color: 'var(--text-muted)', fontSize: '12px' }}>
                No samples found for this solution. Use manual paste or upload.
              </div>
            ) : (() => {
              // Flat list sorted by log type, with source badges
              const sorted = [...browseItems].sort((a, b) => a.logType.localeCompare(b.logType));
              const allSelected = sorted.every((i) => browseSelected.has(i.id));
              const someSelected = sorted.some((i) => browseSelected.has(i.id));
              const tierBadge = (tier: string) => {
                const colors: Record<string, { bg: string; fg: string }> = {
                  'sentinel-repo': { bg: 'rgba(102, 187, 106, 0.15)', fg: 'var(--accent-green)' },
                  elastic: { bg: 'rgba(79, 195, 247, 0.15)', fg: 'var(--accent-blue)' },
                };
                const c = colors[tier] || { bg: 'rgba(255,255,255,0.08)', fg: 'var(--text-muted)' };
                const label: Record<string, string> = { 'sentinel-repo': 'Sentinel', elastic: 'Elastic' };
                return (
                  <span style={{
                    fontSize: '9px', padding: '1px 5px', borderRadius: '3px', fontWeight: 700,
                    textTransform: 'uppercase' as const, background: c.bg, color: c.fg,
                    border: `1px solid ${c.fg}33`, flexShrink: 0,
                  }}>{label[tier] || tier}</span>
                );
              };
              return (
                <div style={{ flex: 1, overflow: 'auto', marginBottom: '16px' }}>
                  ...
                  <div style={{
                    ...
                    <input type="checkbox"
                      ...
                      onChange={() => {
                        if (allSelected) setBrowseSelected(new Set());
                        else setBrowseSelected(new Set(sorted.map((i) => i.id)));
                      }}
                    />
                    ...
                    <span style={{ flex: 1 }} />
                    <span style={{ fontSize: '11px', fontWeight: 600, color: 'var(--text-secondary)', width: '60px' }}>Source</span>
                    <span style={{ fontSize: '11px', fontWeight: 600, color: 'var(--text-secondary)', width: '60px' }}>Format</span>
                    <span style={{ fontSize: '11px', fontWeight: 600, color: 'var(--text-secondary)', width: '55px', textAlign: 'right' }}>Events</span>
                  </div>
                  {sorted.map((item) => (
                    <div key={item.id}>
                      <div style={{
                        ...
                        <input type="checkbox"
                          checked={browseSelected.has(item.id)}
                          onChange={() => {
                        ...
                        />
                        <span style={{ fontWeight: 600, color: 'var(--text-primary)', minWidth: '120px' }}>
                          {item.logType}
                        </span>
                        <span style={{ flex: 1, color: 'var(--text-muted)', fontSize: '10px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                          {item.fileName}
                        </span>
                        <span style={{ width: '60px' }}>{tierBadge(item.tier)}</span>
                        <span style={{ width: '60px', color: 'var(--text-muted)', fontFamily: 'var(--font-mono)', fontSize: '10px' }}>
                          {item.format.toUpperCase()}
                        </span>
                        <span style={{ width: '55px', textAlign: 'right', color: 'var(--text-secondary)', fontFamily: 'var(--font-mono)', fontSize: '10px' }}>
                          {item.eventCount}
                        </span>
                      </div>
                      {browseSelected.has(item.id) && item.preview && item.preview.length > 0 && (
                        <div style={{
                          marginLeft: '28px', marginBottom: '4px', padding: '4px 8px',
                          background: 'var(--bg-primary)', borderRadius: '3px',
                          fontFamily: 'var(--font-mono)', fontSize: '9px',
                          color: 'var(--text-secondary)', maxHeight: '36px', overflow: 'hidden',
                          whiteSpace: 'pre', textOverflow: 'ellipsis',
                          border: '1px solid var(--border-color)',
                        }}>
                          {item.preview.slice(0, 2).map((line: string) =>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '12px', borderTop: '1px solid var(--border-color)' }}>
              <div style={{ fontSize: '11px', color: 'var(--text-muted)' }}>
                {browseSelected.size > 0 ? `${browseSelected.size} sample(s) selected` : 'Select samples to load'}
              </div>
              <div style={{ display: 'flex', gap: '8px' }}>
                <button className="btn-secondary" style={{ fontSize: '12px', padding: '6px 16px' }}
                  onClick={() => setBrowseOpen(false)} disabled={browseCommitting}>
                  Cancel
                </button>
                <button className="btn-primary" style={{ fontSize: '12px', padding: '6px 16px' }}
                  onClick={handleCommitBrowseSelection}
                  disabled={browseSelected.size === 0 || browseCommitting}>
                  {browseCommitting ? 'Loading...' : `Load ${browseSelected.size} Selected`}
                </button>
              </div>
            </div>
          </div>
      ...
      {(analyses.length > 0 || analyzing) && (
        <div style={{ ...s.section, borderLeft: '3px solid var(--accent-blue)' }}>
          ...
          {/* Table resolution info */}
          {tableResolution && !analyzing && (
            <div style={{
              padding: '8px 12px', marginBottom: '12px', borderRadius: '4px', fontSize: '11px',
              background: tableResolution.source.includes('Default')
                ? 'rgba(255, 167, 38, 0.08)' : 'rgba(102, 187, 106, 0.08)',
              border: `1px solid ${tableResolution.source.includes('Default')
                ? 'rgba(255, 167, 38, 0.2)' : 'rgba(102, 187, 106, 0.2)'}`,
              color: tableResolution.source.includes('Default')
                ? 'var(--accent-orange)' : 'var(--accent-green)',
            }}>
              <span style={{ fontWeight: 600 }}>Destination: </span>
              <span style={{ fontFamily: 'var(--font-mono)' }}>{tableResolution.tables.join(', ')}</span>
              <span style={{ color: 'var(--text-muted)', marginLeft: '8px' }}>-- {tableResolution.source}</span>
            </div>
          )}
              ...
              <div style={s.field}>
                ...
                <select style={s.select} value={state.subscription}
                  onChange={async (e) => {
                    ...
                    update({ subscription: subId, workspace: '', resourceGroup: '', location: '' });
                    if (subId && window.api) {
                      try {
                        const [wsResult, rgResult] = await Promise.all([
                          window.api.auth.azureWorkspaces(subId),
                          window.api.auth.azureResourceGroups(subId).catch(() => ({ success: false, resourceGroups: [] as Array<{name: string; location: string}> })),
                        ]);
                        if (wsResult.success) {
                          setWorkspaces(wsResult.workspaces);
                          if (!rgResult.success || rgResult.resourceGroups.length === 0) {
                ...
                <select style={s.select} value={state.workspace}
                  onChange={(e) => {
                    ...
                    update({
                      ...
                      resourceGroup: ws?.resourceGroup || state.resourceGroup || '',
                      location: ws?.location || state.location || '',
                    });
                  }}
