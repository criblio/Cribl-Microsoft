...
- RESOLVED (verified 2026-07-02, Live Preview): KV DELETE IS processed server-side but the bridge never returns the response (keys verified gone after the timeout). The harness times each DELETE out after 8s and re-lists keys to confirm. Two derived rules now baked into the code: all KV deletes go through a race-timeout kvDelete helper that treats timeout as success, and deletes are fired independently (never sequenced on each other's response). Also verified: the platform fetch bridge IGNORES AbortSignal, so all client-side timeouts must be Promise.race based. Still worth one re-test after Deploy to see whether installed mode returns DELETE responses normally (report the bridge bug to Cribl either way).
