...
| Sample Data: Upload Files / Browse Samples | Upload works (browser File API); Browse pulled from the local repo mirror | File input for upload (fine in iframe). Browse was built as an on-demand fetch from the content port and then REMOVED - ADR-0003, 2026-08-18: samples come from the operator, by Lake query, filtered capture or upload, with the log types to bring recommended from their own environment | Unit 11; Unit 16 RETIRED |
