...
- **DBT-28** The solution deep link does not override a stored selection
  ...
  stale: [[DBT-96]]. SECOND ATTEMPT BUILT AND REVERTED 2026-09-04, and the
  revert is a decision about the APPROACH rather than a verdict on the code.
  The attempt read the deep link on every hashchange instead of only on mount,
  which closes defect (2) - and review measured that it reopens the 2026-07-08
  Clear-selection regression through exactly the door the FIRST attempt's
  review warned about. MEASURED, not argued: select a solution, Clear it, then
  press the browser BACK BUTTON with no app code running - the hash returns to
  `#/?solution=<name>`, the solution re-selects and onSelect fires. Worse,
  Back after a pivot X to Y reaches handleSolutionChange with prevName Y and
  nextName X, taking the sample-DELETE branch, so Back silently destroys every
  sample acquired for Y. Before the change Back did nothing at all, because
  the hash was read on mount only. Every `location.hash =` PUSHES a history
  entry and nothing in the repo uses replaceState, so the entries accumulate.
  THE JUSTIFYING COMMENT WAS FALSE, which is review finding (b) from attempt 1
  repeating verbatim: it argued 'a hashchange fires only when someone WRITES a
  different hash, and the only writers are select(), clearSelection() and the
  pivot - all user-driven.' A probe fired one from history.back() with no
  writer. That sentence was the whole argument for choosing hashchange over a
  route-entry read, so the card's design question was answered from a false
  premise - and it propagated into the operator-visible Clear warning, which
  now enumerates 'the two ways to move to another solution' when there are
  three. TWO MORE, both measured: the mount path still parks the name in state
  and applies it in an effect declared AFTER the hash-apply effect, so a NEWER
  deep link arriving while the index is in flight is applied and then
  REVERTED, with the URL rewritten backwards to the stale name - the exact
  failure the new design claimed to have eliminated. And the unresolved-
  handoff notice, which is the whole answer to 'the silence is closed', sits
  inside NumberedSection's body, which is `hidden` when collapsed - the state
  section 1 is in exactly when a solution is selected, which is the only state
  where the defect is reachable. WHAT THE TWO ATTEMPTS BOUGHT, and it is the
  reason to stop patching: THE URL IS THE WRONG CHANNEL. Both attempts failed
  on the same property - a hash is global, traversable browser state that the
  component cannot attribute, so it cannot tell an operator's pivot from a
  history traversal. The reviewer's route: STOP USING THE URL AS THE HANDOFF.
  The pivot already calls onOpenIntegration?.(), so pass the solution name
  through shell state or props and DELETE the listener. If the hash must stay,
  select()/clearSelection() should write via history.replaceState so no
  traversable entries accumulate, and the pivot needs an explicit in-document
  signal instead of a hash write. DEFECT (1) IS UNTOUCHED AND STILL THE
  HEADLINE: resolveSelectedSolution matches exact and case-insensitive-exact
  only, not the punctuation-collapsing match the search box uses, so a deep
  link naming a solution that exists under different punctuation resolves to
...
- **DBT-77** AWS VPC Flow Logs are classified supported but the parser reads none of them
  ...
  Decide which before building. DONE 2026-09-02, all three options rather than
  one. (1) A positional SampleFormat member with generic whitespace splitting,
  so any fixed-column log parses instead of falling through to unknown. (2)
  VPC Flow v2 recognition keyed on the shape, naming the 14 real fields and
  DECLINING to name anything it does not recognise - the reported file now
  parses as 22 events with version, account-id, interface-id, srcaddr,
  dstaddr, srcport, dstport, protocol, packets, bytes, start, end, action,
  log-status. (3) An honest note replacing the misleading error: an
  unrecognised positional file says it was READ as a positional log with N
  columns and that positional formats keep their names outside the file,
  rather than could not parse any events, which read like a corrupt upload.
  The classification claim is now TRUE rather than needing to be withdrawn.
  THE REPORTED FILE CORRECTED THE FIRST DRAFT, which is the part worth
  keeping. It required action to be ACCEPT or REJECT and the numerics to be
  digits. The sample has one legitimate row - 2 acct eni - - - - - - - start
  end NODATA SKIPDATA - using AWS documented dash placeholder for fields
  unavailable on a record, and that single row made all 22 lines decline to be
  named. Numerics now accept the dash, and the action check was DROPPED rather
  than widened because log-status already carries the closed vocabulary, so
  action added no discrimination while being the thing that failed. TWO
  EXISTING CHARACTERIZATION PINS THEN CAUGHT THE DETECTOR BEING GREEDY, and
  they were right: a one-row rule claimed plain prose and an unparseable brace
  as positional logs, because a single row is trivially consistent with
  itself. Detection now needs at least TWO rows before consistency counts as
  evidence. Neither pin was weakened - the code was. Mutation-checked in both
  directions: reinstating the digits-only rule fails the dash pin, dropping
  the 14-column guard fails the decline pin, and allowing a single row fails
  all three including both characterization pins. VERIFIED LIVE 2026-09-02 by
  the reporting user: re-uploading the sample and rebuilding the pack, the
  pipeline renames correctly in Cribl and the accessor error is gone. AND THE
  USER CORRECTED MY DIAGNOSIS MID-FIX, which is the part worth keeping. I had
  concluded the generated pipeline lacked an extraction step for positional
  data - that Cribl received the whitespace line, created no fields, and the
  rename then referenced fields that never existed. I wrote 57 lines of
  positional extraction into pipeline-conf on that basis. The user said the
  account-id string was in the raw sample and they could see it on screen, and
  opening the pack in Cribl settled it: _raw is NOT the positional line, it is
  JSON. The app re-serialises parsed events to JSON and uploads THAT as the
  sample file, so the hyphenated names were baked into _raw and Cribls JSON
  Parser extracted them faithfully. Extraction reads a JSON KEY, where any
  string is legal; rename needs a property ACCESSOR PATH, where account-id
  reads as account minus id. That is the whole bug, and it is exactly what the
  user described. The 57 lines were REMOVED rather than kept as insurance: the
  JSON Parser is correct for this path, and a whitespace split over a JSON
  _raw would have broken something that works. The lesson is narrow and worth
...
- **DBT-80** parseCef's extension parser truncates keys and swallows the pairs that follow
  ...
  first. DONE 2026-09-04, in two rounds, and the first round shipped THREE new
  silent defects while closing the reported one. That is the record worth
  keeping. THE CAUSE, measured rather than assumed. The old pattern carried
  TWO word-class atoms that fail differently and are separately pinnable: the
  CAPTURE truncates the name (surveyed one character at a time, all 29
  punctuation characters came back truncated), and the LOOKAHEAD decides where
  a value ENDS, so a next key that is not all word characters is not a visible
  boundary and the value expands straight through it. POSITION decides which
  half fires: a broken key FIRST only truncates; from second position on it
  also swallows. A survey that only put the odd key first would have reported
  'truncation, count preserved' and missed the field loss entirely. THE CARD'S
  OWN STRONGEST HYPOTHESIS WAS REFUTED: the escaped equals is NOT the cause.
  Measured on HEAD, `msg=x y\=z dst=2.2.2.2` pairs correctly, because a
  backslash is not a word character so the old lookahead could never read it
  as a boundary. What the escape actually does is CONSTRAIN the fix - a key
  class wide enough to include a backslash manufactures a field out of the
  escape - and it is pinned as a control, not as a fix. ROUND 1'S THREE
  DEFECTS, all silent, all under a green suite: (1) THE WORST THING IN THE
  WAVE, and not loss but a WRONG VALUE reported as correct. The
  leading-grouping trim borrowed from parseKv turned the prose token `(act`
  into the key `act`, and assignment is last-write-wins, so `act=deny
  msg=blocked (act=drop)` reported act="drop)" where the device said deny.
  Same field names, same count, zero notes. (2) A value containing `<=`, `(=`,
  `[=` or `{=` opened a pair whose key trimmed to EMPTY; the empty-key guard
  dropped it and the SPANNING value took the rest of the message with it.
  `msg=Disk usage <= 90 percent on /var` lost everything after 'usage',
  appearing in no field. (3) The comment block's safety argument was false in
  both halves - it could lose text rather than only gain a field, and the
  invented name is usually a perfectly good accessor (`retries`, `action`,
  `reason`, `src`), so DBT-78's note does NOT fire. The grouping trim was what
  MADE those names accessor-safe. THE CORPUS LESSON, and it is the third time
  in this session. Round 1 reported 0 lines with fewer keys than HEAD. An
  independent reviewer's corpus found 319 lines with fewer keys and 24,367
  HEAD keys lost. The generator was not dishonest - it never emitted a bare
  grouping token before an equals. A CORPUS THAT CANNOT EXPRESS THE FAILURE
  CANNOT CLEAR IT, and [[DBT-79]] was cleared twice the same way. Round 2
  regenerated with the four grouping openers reachable as complete key tokens
  and now quotes its CALIBRATION beside its number. SHIPPED: the grouping trim
  is GONE from parseCef (chosen over an overwrite guard, because that guard
  only protects when the real field appears BEFORE the parenthetical, and
  keeping the bracket is also what makes the case LOUD - the note now fires on
  `(act`); the key must contain a word character in BOTH the capture and the
  lookahead (measured: each half does a different job, lookahead-only leaves
  81,701 lines with a punctuation-named field, capture-only leaves 80,978
  losing value text); and lines split on an optional carriage return. A
  PRE-EXISTING CRLF DEFECT WAS FOUND AND FIXED ACROSS FIVE PARSERS. parseCef
  dropped the LAST extension pair of every CRLF line - masked end to end
  because parseSampleContent UNIONS field names across records, so the last
  record contributed the missing name while record 0 lacked it. Checking the
  siblings cost the agent its own first answer: parseLeef welded the carriage
  return into the last VALUE and parseSyslog into `_raw`; parseKv and parseCsv
  were written up as clean and are clean only in their FIELDS - both still
  pushed the carriage return into rawEvents, which is what the generated pack
  is previewed and shipped against. Only parseNdjson was genuinely unaffected.
  Follow-ups: [[DBT-98]] (the header split ignores CEF's own pipe escape),
  [[DBT-99]] (values keep their escape sequences unexpanded).
...
- **GEN-4** A field name containing whitespace passes checkCriblYaml even on a rename line
  ...
  same rule. DONE 2026-09-04. checkCriblYaml now catches a field name
  containing WHITESPACE on a rename line, so a rename Cribl cannot address no
  longer ships with a green build. THE REAL WORK WAS THE DISTINCTION, not the
  regex, exactly as the card predicted. The old value class was load-bearing:
  it was the only thing stopping the rule flagging the three group headers
  every generated conf carries ('Field Extraction', 'Enrich & Classify',
  'Sentinel Cleanup'), and widening the character class alone produces exactly
  3 false issues on two existing generated-pack tests. Selection is now by
  ENCLOSING BLOCK rather than by value shape, which is what lets the class
  widen safely - and it is reusable by [[GEN-5]], which needs the same
  distinction. THE FIX OPENED A FAIL-OPEN HOLE AND REVIEW CAUGHT IT BEFORE IT
  COMMITTED. The widened matcher's tail could not consume a carriage return -
  a JavaScript line terminator - so on CRLF content the matcher returned null
  for EVERY name line and the rule reported the file CLEAN. Measured 0 issues
  where LF gave 1, for a hyphenated and a dotted name alike. The pre-GEN-4
  regex had survived CRLF BY ACCIDENT. So the change traded a whitespace hole
  for a line-ending one, in the direction this validator exists to prevent.
  BOTH GUARDS SHIP, and the argument for shipping only one was measured FALSE.
  The repair normalised line endings once at the top and argued that ALSO
  restoring the tail 'would make removal of the normalisation undetectable by
  any pin'. Review measured otherwise - with the tail restored, deleting the
  normalisation still fails the lone-carriage-return route pin - and found the
  residual that argument left open: U+2028 and U+2029 are line terminators the
  SPLIT does not break on, so normalisation never reaches them and the rule
  read nothing. Reachability is low (no emitter writes them; YAML 1.2 does not
  treat them as breaks) but the direction is fail-open, and 'it fixes the
  class' was the claim that left it there. Two cheap guards covering different
  sets beat one elegant guard covering most of them. THIS CHANGE TURNED THE
  TREE RED ON PURPOSE, which is the system working: accessor-names.test.ts:107
  was a deliberate TRIPWIRE whose own body read 'when this pin fails, that is
  the good outcome: someone taught the validator whitespace names. Re-read the
  note's last sentence at the same time.' It fired, and the re-read was the
  work. Still open and NOT closed here: [[GEN-5]], the wider hole in the same
  rule - an unmatched or kept field emits no rename line at all, so the
  validator never sees the name.
...
- **GEN-6** A positional sample generates a pipeline whose extract step JSON-parses the line
  ...
  [[DBT-78]]'s closing note and should be read first. DONE 2026-09-04, and
  this closes the defect a user actually reported, end to end. [[DBT-77]] made
  an AWS VPC Flow Logs sample PARSE. pipeline-conf.ts had no positional
  branch, so the generated pack's extract step ran Cribl's JSON serde over
  `_raw` - and a whitespace-positional line is not JSON, so it extracted
  nothing. The app showed success at every screen and handed the operator a
  pack that could not reproduce any of it. SHIPPED: a positional branch that
  splits `_raw` itself and assigns the names
  domain/sample-parsing/positional.ts produced - the recognised v2 fields with
  underscores, or field1..fieldN. This is the case where WE mint the runtime
  name, so the parser's names and the pipeline's names are the same names BY
  CONSTRUCTION, which is exactly the distinction [[DBT-78]] established and
  the reason sanitising is correct here and wrong for JSON/KV. AWS's
  hyphenated spellings stay in VPC_FLOW_V2_AWS_NAMES for display and never
  reach the pipeline. THE PACK'S OWN SAMPLE DATA DEFEATED IT, and review
  caught that. parseByFormat threads `sourceLines` into every other
  line-oriented parser and NOT into the positional case, so rawEventsFor's
  length check failed and fell back to JSON-stringifying each record - which
  pack-assembly then writes as the pack's `_raw`. Running the new extract over
  that shipped sample yielded 1 field of 14, because JSON output has no
  whitespace. Before this branch existed the JSON serde happened to MATCH that
  mis-serialized sample, so the in-pack preview looked right while real data
  produced nothing; the fix inverted the signal rather than making it honest.
  parse-sample.ts's own header already called this out for LEEF, syslog and
  PAN-OS CSV - 'the pack's own pipeline previewed against data shaped nothing
  like what the source actually sends' - and positional had been left out of
  that fix. Now threaded. ALSO FIXED: a multi-log-type positional pack got
  route filters that CAN NEVER MATCH. Routes run BEFORE the pipeline extracts,
  so at route time a positional event is exactly as unparsed as a CSV one -
  neither the field nor the literal text can exist. formatCanDiscriminate
  tested only for CSV, so every disjunct was false, the route dead-ended, and
  because a filter WAS produced the log type counted as neither placeholder
  nor unreachable, so preview reported a clean pack and HON-5 told the
  operator to collect more samples, which could not help. The predicate is
  deliberately the single authority and was widened there. VERIFIED END TO END
  on AWS's own published v2 examples including the all-dashes NODATA/SKIPDATA
  row: parse gives 14 named fields with no note; the emitted conf's extract
  names match the parser's exactly, in order; checkCriblYaml passes; and the
  pack's rawEvents are the ORIGINAL whitespace lines, so the shipped sample
  now extracts 14 fields rather than 1. NOT verified in a live Cribl - the
  pins execute the emitted expressions as JavaScript, which is what Cribl's
  Eval does, and the conf passes the acceptance validator, but nobody
  installed it. Follow-up: [[GEN-7]] (a recognised v2 pack stamps ingestion
  time, not flow time).
...
- **DBT-90** The root package.json never forwarded check-release, so the documented command exits 1
  ...
  workspace. DONE 2026-09-04. The root package.json now forwards
  check-release, so the command the docs tell you to run locally actually runs
  instead of exiting 1 with an npm error. THE SWEEP WAS THE WORK, not the
  one-line script - and it found the card's own failure mode twice, in prose
  written while fixing it. (1) The first draft said 33 class-name findings
  'accumulated in its place' after DBT-39. FALSE. A reviewer extracted the
  tree at e147332 - the commit that ADDED check-classnames.mjs - ran the
  script there and on the current tree, and diffed the name sets:
  byte-identical, 36 findings across 33 distinct names then and now, zero
  added, zero removed in three days. Corroborated by introduction dates: of 15
  sampled names, 14 predate the script and 11 are from July. Nothing
  accumulated; the four DBT-39 names were fixed in the same commit and the
  other 33 were left standing. (2) The draft asserted the script had 'no
  mention in any document' - a sentence which IS that mention, so it was false
  the moment it was committed, and a single grep refuted the card wording that
  restated it. Both corrected against measurement rather than reworded. The
  load-bearing part survives and was independently verified: no manifest
  defines a check-classnames npm script and CI has no such step, so nothing
  ever invokes it - which is [[DBT-100]]. NOT CLOSED: no pin guards the new
