/**
 * Source Type Knowledge Base + inputs.yml generator - porting-plan Unit 17
 * (ENG-13), task item (e).
 *
 * Ported VERBATIM from legacy IS/source-types.ts (1278 lines): the nine source
 * type definitions (syslog, REST collector, HTTP, Azure Event Hub, Azure Blob,
 * Kafka, Office 365, S3, Windows Events) with their fields/presets/discovery,
 * the vendor-source hint map, suggestSourceType, and generateInputsYml with its
 * merge order and formatYamlValue quoting rules.
 *
 * ONE deliberate rename vs legacy: the field-descriptor interface `SourceField`
 * is renamed {@link SourceTypeField} here. Legacy `SourceField` collides with
 * the field-matcher/sample domain `SourceField` (a source LOG field). This one
 * describes a UI/config FORM field of a Cribl input; the rename removes the
 * package-root barrel clash. All catalog DATA and generateInputsYml behavior are
 * byte-identical to legacy.
 *
 * Pure: no IO, no fetch, no React, no Date/crypto/Math.random.
 */

/** One UI/config form field of a Cribl input definition (legacy: SourceField). */
...
const syslog: SourceTypeDefinition = {
  ...
  name: "Syslog",
  description:
    "Receive syslog messages over TCP or UDP. Simplest source type -- just configure port and protocol.",
  criblType: "syslog",
  category: "push",
  fields: [
    {
    ...
    },
    {
      ...
      default: false,
      description:
        "Enable HAProxy PROXY protocol to preserve original client IP.",
      group: "Advanced",
    },
  ],
  staticConfig: {
  ...
  vendorPresets: {
    paloalto: {
      ...
      description:
        "Palo Alto firewall syslog (CEF/LEEF format, typically TCP 514)",
...
const restCollector: SourceTypeDefinition = {
  ...
  name: "REST Collector",
  description:
    "Poll a REST API endpoint on a schedule. Supports pagination, authentication, and optional discovery of endpoints.",
  criblType: "rest",
  category: "pull",
  fields: [
    {
      ...
      description:
        "URL to poll for log data. Supports Cribl expressions for dynamic URL construction.",
      group: "Collection",
    },
    {
      name: "collectMethod",
    ...
    },
    {
      ...
      placeholder: "*/5 * * * *",
      description:
        "Cron expression for collection interval (default: every 5 minutes).",
      group: "Collection",
    },
    {
      name: "authentication",
    ...
    },
    {
      ...
      default: 100,
      description:
        "Maximum pages to fetch per collection cycle (safety limit).",
      group: "Pagination",
    },
    {
      name: "timeout",
    ...
    },
    {
      ...
      default: true,
      description:
        "Verify the server TLS certificate. Disable for self-signed certs.",
      group: "Advanced",
    },
  ],
  discovery: {
    enabled: true,
    description:
      "Some REST APIs require a discovery phase to enumerate available log types, tenants, or endpoints before collection.",
    fields: [
      {
        name: "discoverUrl",
        label: "Discovery URL",
        ...
        description:
          "URL that returns a list of available log sources or endpoints.",
        group: "Discovery",
      },
      {
        name: "discoverMethod",
      ...
      },
      {
        ...
        description:
          "Dot-notation path to the array of discovered items in the response.",
        group: "Discovery",
      },
      {
        ...
        placeholder: "id",
        description:
          "Field within each discovered item to use as the unique identifier.",
        group: "Discovery",
      },
    ],
  },
  ...
  vendorPresets: {
    cloudflare: {
      ...
      fieldDefaults: {
        collectUrl:
          "https://api.cloudflare.com/client/v4/zones/${zoneId}/logs/received",
        collectMethod: "get",
    ...
    crowdstrike: {
      ...
      fieldDefaults: {
        collectUrl:
          "https://api.crowdstrike.com/sensors/entities/datafeed/v2",
        collectMethod: "get",
    ...
    microsoft_graph: {
      ...
      description:
        "Microsoft Graph API for security alerts, incidents, and sign-in logs",
      fieldDefaults: {
    ...
    qualys: {
      ...
      description:
        "Qualys API for vulnerability scan results and host detection",
      fieldDefaults: {
        collectUrl:
...
const httpSource: SourceTypeDefinition = {
  ...
  name: "HTTP / Webhook",
  description:
    "Receive events via HTTP POST. Supports Splunk HEC format, generic JSON, and webhook payloads.",
  criblType: "http",
  category: "push",
  fields: [
    {
    ...
    },
    {
      ...
      required: false,
      description:
        "Token for Splunk HEC-style auth (Authorization: Splunk <token>).",
      group: "Authentication",
    },
    {
      name: "maxActiveReq",
  ...
  vendorPresets: {
    splunk_hec: {
      ...
      description:
        "Accept events from Splunk Universal Forwarders or HEC-compatible senders",
...
const azureEventHub: SourceTypeDefinition = {
  ...
  description:
    "Consume events from Azure Event Hubs. Used for Azure Activity Logs, Diagnostic Logs, and any Azure service that exports to Event Hub.",
  ...
  fields: [
    {
      ...
      description:
        "Event Hub namespace connection string with Listen permission.",
      group: "Connection",
    },
    {
      name: "eventHubName",
    ...
    },
    {
      ...
      default: "$Default",
      description:
        "Event Hub consumer group. Use a dedicated group for Cribl to avoid conflicts.",
      group: "Connection",
    },
    {
      ...
      description:
        "Azure Storage account for checkpointing (tracking read position).",
      group: "Checkpoint",
    },
    {
  ...
  vendorPresets: {
    ...
    azure_diagnostics: {
      ...
      fieldDefaults: {
        eventHubName: "insights-logs-diagnostics",
        consumerGroup: "cribl",
      },
    },
    azure_nsg: {
      ...
      fieldDefaults: {
        eventHubName: "insights-logs-networksecuritygroupflowevent",
        consumerGroup: "cribl",
      },
    },
  },
...
const azureBlob: SourceTypeDefinition = {
  ...
  description:
    "Collect log files from Azure Blob Storage containers. Supports scheduled polling or event-driven collection via Event Grid.",
  ...
  vendorPresets: {
    azure_flow_logs: {
      ...
      fieldDefaults: {
        path: "insights-logs-flowlogflowevent/",
...
const kafka: SourceTypeDefinition = {
  ...
  fields: [
    ...
    },
    {
      ...
      default: false,
      description:
        "Start reading from the beginning of the topic (otherwise latest).",
      group: "Advanced",
    },
  ],
  staticConfig: {
...
const office365: SourceTypeDefinition = {
  ...
  description:
    "Collect Office 365 Management Activity API logs. Requires Azure AD app registration with Office 365 Management API permissions.",
  ...
  discovery: {
    enabled: true,
    description:
      "O365 Management API uses a subscription model. The discovery phase creates/verifies content type subscriptions before collection begins.",
    fields: [
      {
        name: "autoSubscribe",
        ...
        default: true,
        description:
          "Automatically create subscriptions for the selected content types.",
        group: "Discovery",
      },
    ],
  },
...
const s3: SourceTypeDefinition = {
  ...
  name: "AWS S3",
  description:
    "Collect log files from AWS S3 buckets. Supports SQS-based notification for real-time collection.",
  criblType: "s3",
  category: "pull",
  fields: [
    {
    ...
    },
    {
      ...
      description:
        "SQS queue URL for event-driven collection (instead of polling).",
      group: "Collection",
    },
    {
      name: "schedule",
...
const windowsEvents: SourceTypeDefinition = {
  ...
  description:
    "Collect Windows Event Logs locally or via Windows Event Forwarding (WEF). Requires Cribl Edge agent on Windows hosts.",
  ...
  fields: [
    {
      ...
      description:
        "Comma-separated list of Windows event channels to collect.",
      group: "Collection",
    },
    {
      name: "eventFilter",
    ...
    },
    {
      ...
      default: false,
      description:
        "Read existing events when starting (otherwise start from current position).",
      group: "Advanced",
    },
  ],
  staticConfig: {
  ...
  vendorPresets: {
    security_focused: {
      ...
      fieldDefaults: {
        channels:
          "Security,Microsoft-Windows-Sysmon/Operational,Microsoft-Windows-PowerShell/Operational,Microsoft-Windows-Windows Defender/Operational",
      },
    },
    all_channels: {
      ...
      fieldDefaults: {
        channels:
          "Security,System,Application,Setup,Microsoft-Windows-Sysmon/Operational,Microsoft-Windows-PowerShell/Operational",
      },
    },
  },
};
...
export const VENDOR_SOURCE_HINTS: Record<
  string,
...
export function generateInputsYml(
  ...
  if (typeDef.discovery?.enabled && config.discoveryFields) {
    ...
    if (
      config.vendorPreset &&
      typeDef.vendorPresets?.[config.vendorPreset]?.discoveryDefaults
    ) {
      Object.assign(
        discoverFields,
        typeDef.vendorPresets[config.vendorPreset].discoveryDefaults,
