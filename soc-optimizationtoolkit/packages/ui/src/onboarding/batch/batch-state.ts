...
export function formatBatchSummary(
  ...
  if (templateOnly) {
    lines.push(
      `${outcome.templates.length} resource(s) collected into one ARM ` +
        "deployment template - download it below",
    );
  }
...
/**
 * Serialize the collected requests as ONE ARM DEPLOYMENT TEMPLATE (DBT-33).
 *
 * This used to emit a wrapper holding each REST request verbatim - method,
 * path, api-version, body. That is what the usecase collects, and it deploys
 * through `az rest`, but it is not what "export ARM templates" means to
 * anyone who then tries to use it: Portal's custom-template blade and
 * `az deployment group create` both reject a file with no `$schema` and no
 * `resources[]`. Now it emits the real envelope, so the artifact matches its
 * own file name.
 *
 * The collected metadata is not lost - `_criblToolkit` keeps the scope the
 * template must be deployed to and anything the build could not include, in
 * a leading-underscore key ARM ignores. Deterministic: no timestamps, no
 * random ids.
 */
export function buildTemplatesArtifact(
  templates: readonly CollectedArmRequest[],
): string {
  const build = buildArmTemplate(templates);
  return JSON.stringify(
    {
      ...
      _criblToolkit: {
        note:
          ...
          "Deploy with: az deployment group create --subscription " +
          `${build.subscriptionId || "<subscription>"} --resource-group ` +
          `${build.resourceGroup || "<resource-group>"} --template-file <this file>`,
        subscriptionId: build.subscriptionId,
        resourceGroup: build.resourceGroup,
        // Non-empty means resources were EXCLUDED: they sit outside the one
        // resource group this deployment targets, or named no resource.
        scopeConflicts: build.scopeConflicts,
        unparseable: build.unparseable,
      },
    },
    null,
    2,
