...
export function RepositoriesScreen({ platform }: RepositoriesScreenProps) {
  ...
  const llmKey = ports.llmKey;

  // ---- AI Assist (Anthropic) key lifecycle -------------------------------
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const status = await llmKey.status();
        if (!cancelled) setAiHasKey(status.hasKey);
      } catch {
        // A failed status read leaves the section in its unknown default.
      }
    })();
    return () => {
      cancelled = true;
  ...
  const saveAiKey = useCallback(async () => {
    if (llmKey === undefined || aiKeyInput.trim() === "") {
      return;
    }
    setAiBusy(true);
    setAiError("");
    try {
      const status = await llmKey.validateAndStore(aiKeyInput);
    ...
    <div className="repositories-screen">
      ...
      {llmKey !== undefined && (
        <section className="numbered-section">
          <div className="numbered-section-head">
            <span
              className={`numbered-section-badge ${
                aiHasKey === true
                  ? "numbered-section-badge-complete"
                  : "numbered-section-badge-current"
              }`}
            ...
            <h2 className="numbered-section-title">
              AI Assist (Anthropic API key)
            </h2>
          </div>
          <p className="panel-desc">
            Optional: an Anthropic API key enables AI suggestions in the DCR Gap
            Analysis and the rule/workbook coverage panels (model: Fable 5).
            Advisory only - the deterministic analysis is always the source of
            truth, nothing changes until you apply a suggestion, and no deploy
            gate ever reads AI output. Prompts carry field names, types, and at
            most one truncated example value - never secrets or raw events.
            Without a key the app works exactly as before.
          </p>
          {aiHasKey === true && (
            <div className="status-bar status-bar-ready">
              <span className="status-bar-dot" aria-hidden="true" />
              <span className="status-bar-text">Anthropic key saved</span>
              <button
                className="run-button status-bar-action"
          ...
          <div className="form-grid">
            <label className="field">
              ...
              />
              <span className="field-hint">
                The key is validated against Anthropic (a zero-token models
                call), then stored encrypted and write-only - it is never shown
                again and never reaches the browser.
              </span>
            </label>
          </div>
          <div className="panel-controls">
            ...
            >
              {aiBusy ? "Validating..." : "Validate and save"}
            </button>
          </div>
          {aiError !== "" && <pre className="result">{aiError}</pre>}
        </section>
      )}
    </div>
  );
