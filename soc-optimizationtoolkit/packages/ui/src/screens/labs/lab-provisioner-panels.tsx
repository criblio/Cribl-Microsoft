/**
 * LabProvisionerPanels - the plan-and-deploy surface (panels 1-4): profile
 * selection, target/naming/TTL, plan review with the permission check and
 * plan download, and the live deploy over the @soc/core provisionLab engine.
 * The FORM and PLAN are screen-owned props (the flow-log pack panel shares
 * the planned storage name); this component owns only its IO state.
 */
...
export interface LabProvisionerPanelsProps {
  /** The screen-owned form (shared so the plan has one source). */
  form: LabFormState;
  /** Update one form field. */
  onFormChange: <K extends keyof LabFormState>(key: K, value: LabFormState[K]) => void;
  /** The plan derived from the form (labPlanFromForm, screen-owned). */
  plan: LabPlan;
  /** The screen-owned subscription the plan/check/deploy target. */
  subscriptionId: string;
  /** The last deploy outcome (screen-owned; the pack panel reads it too). */
  deployResult: ProvisionLabResult | null;
  /** Store a deploy outcome (or clear it when a new deploy starts). */
  onDeployResult: (result: ProvisionLabResult | null) => void;
}

export function LabProvisionerPanels({
  form,
  onFormChange,
  plan,
  subscriptionId,
  deployResult,
  onDeployResult,
}: LabProvisionerPanelsProps) {
  const { ports, config } = usePorts();
  const set = onFormChange;
    ...
    try {
      const outcome = await provisionLab(
        { azure: ports.azure, jobs: ports.jobs, logger: ports.logger },
        {
          ...
          rgMode: form.rgMode,
          // The SAME parse the plan preview uses - one path, no divergence.
      ...
      onDeployResult(outcome);
    } catch (err) {
      setDeployError(err instanceof Error ? err.message : String(err));
    } finally {
      setDeploying(false);
    }
  }, [deploying, gate.ok, ports, config, plan, form, subscriptionId, onDeployResult]);
