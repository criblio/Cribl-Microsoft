...
function Test-ExistingResources {
    ...
    if ($null -ne $validation.ResourceGroup) {
        ...
        Write-Host "  ℹ️  Resource Group will be created: $ResourceGroupName" -ForegroundColor Cyan
    }
    ...
    if ($null -ne $validation.VNet) {
        ...
        foreach ($subnetKey in $azParams.subnets.PSObject.Properties.Name) {
            ...
            if ($null -ne $existingSubnet) {
                ...
                Write-Host "     ⚠️  Subnet missing: $($subnetDef.name)" -ForegroundColor Yellow
            }
        }
    } else {
        Write-Host "  ℹ️  Virtual Network will be created: $VNetName" -ForegroundColor Cyan
    }
    ...
    if ($null -ne $validation.VPNGateway) {
        ...
        Write-Host "  ℹ️  VPN Gateway will be created: $VpnGatewayName" -ForegroundColor Cyan
    }
    ...
    if ($null -ne $validation.Bastion) {
        ...
        if ($azParams.bastion.deploy) {
            Write-Host "  ℹ️  Azure Bastion will be created: $BastionName" -ForegroundColor Cyan
        }
    }
    ...
    if ($null -ne $validation.StorageAccount) {
        ...
        if ($opParams.flowLogging.createStorageAccount) {
            Write-Host "  ℹ️  Storage Account will be created: $StorageAccountName" -ForegroundColor Cyan
        }
    }
    ...
    if ($null -ne $validation.LogAnalytics) {
        ...
        if ($opParams.flowLogging.createLogAnalyticsWorkspace) {
            Write-Host "  ℹ️  Log Analytics Workspace will be created: $LogAnalyticsName" -ForegroundColor Cyan
        }
    }
    ...
    if ($null -ne $validation.NetworkWatcher) {
        ...
        Write-Host "     ℹ️  Will use existing Network Watcher (only one per region allowed)" -ForegroundColor Cyan
    } else {
        Write-Host "  ℹ️  No Network Watcher found in region '$Location'" -ForegroundColor Cyan
...
function Ensure-ResourceGroup {
    ...
    if ($null -eq $rg) {
        if ($TemplateOnly) {
            Write-Host "  ℹ️  Resource Group '$ResourceGroupName' will be created during deployment" -ForegroundColor Cyan
        } else {
...
function Deploy-VirtualNetwork {
    ...
    if ($null -ne $existingVNet) {
        if ($SkipExisting) {
            Write-Host "  ℹ️  Virtual Network '$VNetName' already exists" -ForegroundColor Cyan
...
function Deploy-NetworkSecurityGroups {
    if (-not $opParams.deployment.deployNSGs) {
        Write-Host "`n  ⏭️  NSG deployment disabled in operation-parameters.json" -ForegroundColor DarkGray
        return
    }
    ...
    foreach ($subnetKey in $azParams.subnets.PSObject.Properties.Name) {
        ...
        if ($subnet.name -eq "GatewaySubnet") {
            Write-Host "  ⏭️  Skipping NSG for $($subnet.name) (not supported)" -ForegroundColor DarkGray
            continue
        }
        ...
        if ($null -ne $existingNSG) {
            Write-Host "  ℹ️  NSG '$nsgName' already exists - skipping" -ForegroundColor Cyan
...
function Deploy-StorageAccount {
    if (-not $opParams.flowLogging.createStorageAccount) {
        Write-Host "`n  ⏭️  Storage Account creation disabled" -ForegroundColor DarkGray
        return $null
    }
    ...
    if ($null -ne $existingSA) {
        Write-Host "  ℹ️  Storage Account '$StorageAccountName' already exists - skipping" -ForegroundColor Cyan
        return $existingSA
    }
        ...
        try {
            ...
            if ($_.Exception.Message -like "*AlreadyExists*" -or $_.Exception.Message -like "*already taken*" -or $_.Exception.Message -like "*not available*") {
                ...
                if ($attemptNumber -lt $maxAttempts) {
                    Write-Host "  ⚠️  Storage Account name '$currentName' is already taken globally" -ForegroundColor Yellow
...
function Deploy-LogAnalyticsWorkspace {
    if (-not $opParams.flowLogging.createLogAnalyticsWorkspace) {
        Write-Host "`n  ⏭️  Log Analytics Workspace creation disabled" -ForegroundColor DarkGray
        return $null
    }
    ...
    if ($null -ne $existingLAW) {
        Write-Host "  ℹ️  Log Analytics Workspace '$LogAnalyticsName' already exists - skipping" -ForegroundColor Cyan
        return $existingLAW
    }
...
function Ensure-NetworkWatcher {
    ...
    try {
        ...
        if ($null -ne $nw) {
            ...
            Write-Host "     ℹ️  Note: Only one Network Watcher per region is allowed" -ForegroundColor Cyan
            return $nw
        }
        ...
        if ($NetworkWatcherRG -eq "USE_MAIN_RG") {
            ...
            Write-Host "  ℹ️  Network Watcher will be created in main resource group: $targetRG" -ForegroundColor Cyan
        } else {
            $targetRG = $NetworkWatcherRG
            Write-Host "  ℹ️  Network Watcher will be created in: $targetRG" -ForegroundColor Cyan
        ...
        Write-Host "  ⚠️  Network Watcher operation failed" -ForegroundColor Yellow
        Write-Host "  Error: $($_.Exception.Message)" -ForegroundColor Yellow
        Write-Host "  ℹ️  Flow Logs may not be available without Network Watcher" -ForegroundColor Cyan
        return $null
    }
}
...
function Deploy-VNetFlowLogs {
    ...
    if (-not $opParams.flowLogging.enableVNetFlowLogs) {
        Write-Host "`n  ⏭️  VNet Flow Logs disabled" -ForegroundColor DarkGray
        return
    }

    Write-Host "`n--- Step 5: Configuring VNet Flow Logs ---" -ForegroundColor Yellow
    Write-Host "  ℹ️  VNet Flow Logs replace deprecated NSG Flow Logs (as of 2024)" -ForegroundColor Cyan
    ...
    if ($null -eq $NetworkWatcher) {
        Write-Host "  ⚠️  Network Watcher not available - skipping VNet Flow Logs" -ForegroundColor Yellow
        return
    }
    ...
    if ($azParams.flowLogging.vnetLevel.enabled) {
        ...
        try {
            ...
            if ($_.Exception.Message -like "*CannotCreateMoreThanOneFlowLogPerTargetResource*") {
                Write-Host "  ℹ️  VNet-level Flow Log already exists - skipping" -ForegroundColor Cyan
            } else {
                Write-Host "  ⚠️  Failed to configure vNet-level Flow Log" -ForegroundColor Yellow
                Write-Host "  Error: $($_.Exception.Message)" -ForegroundColor DarkGray
            }
        }
    } else {
        Write-Host "`n  ⏭️  VNet-level Flow Log disabled" -ForegroundColor DarkGray
    }
    ...
    foreach ($subnetKey in $azParams.subnets.PSObject.Properties.Name) {
        ...
        if ($null -eq $subnetFlowConfig -or -not $subnetFlowConfig.enabled) {
            Write-Host "     ⏭️  $($subnetDef.name): Subnet-level disabled (uses vNet-level if enabled)" -ForegroundColor DarkGray
            continue
        }
        ...
        if ($null -eq $subnet) {
            Write-Host "     ⚠️  $($subnetDef.name): Subnet not found, skipping" -ForegroundColor Yellow
            continue
        }
        ...
        try {
            ...
            if ($_.Exception.Message -like "*CannotCreateMoreThanOneFlowLogPerTargetResource*") {
                Write-Host "        ℹ️  Subnet-level Flow Log already exists - skipping" -ForegroundColor Cyan
            } else {
                Write-Host "        ⚠️  Failed to configure subnet-level Flow Log" -ForegroundColor Yellow
                Write-Host "        Error: $($_.Exception.Message)" -ForegroundColor DarkGray
            }
        }
    }
    ...
    if ($subnetFlowLogsDeployed -gt 0) {
        ...
        Write-Host "     ℹ️  Subnet-level overrides vNet-level for specific subnets" -ForegroundColor Cyan
    }
...
function Deploy-VPNPublicIP {
    ...
    if ($null -ne $existingPip) {
        if ($SkipExisting) {
            Write-Host "  ℹ️  Public IP '$VpnPublicIpName' already exists - skipping" -ForegroundColor Cyan
            return $existingPip
        } else {
...
function Deploy-VPNGateway {
    ...
    if ($null -ne $existingGw) {
        if ($SkipExisting) {
            Write-Host "  ℹ️  VPN Gateway '$VpnGatewayName' already exists - skipping" -ForegroundColor Cyan
            return $existingGw
        } else {
...
function Deploy-Bastion {
    if (-not $azParams.bastion.deploy) {
        Write-Host "`n  ⏭️  Bastion deployment disabled in azure-parameters.json" -ForegroundColor DarkGray
        return $null
    }
    ...
    if ($null -ne $existingBastion) {
        if ($SkipExisting) {
            Write-Host "  ℹ️  Bastion '$BastionName' already exists - skipping" -ForegroundColor Cyan
...
function Deploy-LocalNetworkGateway {
    if ($null -eq $onPremParams) {
        Write-Host "`n  ℹ️  No on-premises connection configuration found - skipping Local Network Gateway" -ForegroundColor Cyan
        return $null
    }
    ...
    if ($null -ne $existingLng) {
        if ($SkipExisting) {
            Write-Host "  ℹ️  Local Network Gateway '$lngName' already exists - skipping" -ForegroundColor Cyan
            return $existingLng
        } else {
    ...
    if ($onPremParams.localNetworkGateway.gatewayIpAddress -like "*YOUR-ONPREM-PUBLIC-IP*") {
        Write-Host "  ⚠️  WARNING: On-premises public IP not configured in onprem-connection-parameters.json" -ForegroundColor Yellow
...
function Deploy-VPNConnection {
    ...
    if ($null -eq $onPremParams) {
        Write-Host "`n  ℹ️  No on-premises connection configuration found - skipping VPN Connection" -ForegroundColor Cyan
        return $null
    }

    if ($null -eq $VpnGateway) {
        Write-Host "`n  ⚠️  VPN Gateway not available - cannot create connection" -ForegroundColor Yellow
        return $null
    }

    if ($null -eq $LocalNetworkGateway) {
        Write-Host "`n  ⚠️  Local Network Gateway not available - cannot create connection" -ForegroundColor Yellow
        return $null
    }
    ...
    if ($null -ne $existingConn) {
        if ($SkipExisting) {
            Write-Host "  ℹ️  VPN Connection '$connName' already exists - skipping" -ForegroundColor Cyan
            return $existingConn
        } else {
    ...
    if ($onPremParams.vpnConnection.sharedKey -like "*YOUR-SHARED-KEY-HERE*") {
        Write-Host "  ⚠️  WARNING: Shared key not configured in onprem-connection-parameters.json" -ForegroundColor Yellow
    ...
    try {
        ...
        Write-Host "  1️⃣  Navigate to: VPN > IPsec > Tunnels > Add P1" -ForegroundColor White
        Write-Host ""
        ...
        Write-Host "  2️⃣  Add Phase 2 (IPsec) Entry:" -ForegroundColor White
        Write-Host ""
        ...
        Write-Host "  3️⃣  Create Firewall Rules:" -ForegroundColor White
        Write-Host ""
        ...
        Write-Host "  4️⃣  Apply and Connect:" -ForegroundColor White
        Write-Host ""
        ...
        if ($null -ne $azureGatewayIp) {
            ...
            Write-Host "`n⚠️  Azure VPN Gateway Public IP not yet available - check Azure Portal" -ForegroundColor Yellow
        }
        Write-Host ""
...
function Deploy-TestVMs {
    if ($null -eq $vmParams -or -not $vmParams.vmConfiguration.deployVMs) {
        Write-Host "`n  ℹ️  VM deployment disabled or no VM configuration found" -ForegroundColor Cyan
        return
    }
    ...
    foreach ($subnetKey in @("bastion", "security", "o11y")) {
        ...
        if (-not $vmConfig.deploy) {
            Write-Host "`n  ⏭️  Skipping VM deployment in $subnetKey subnet" -ForegroundColor DarkGray
            continue
        }
        ...
        try {
            ...
            if ($null -ne $existingVM -and $SkipExisting) {
                Write-Host "  ℹ️  VM '$vmName' already exists - skipping" -ForegroundColor Cyan
            ...
            try {
                ...
                Write-Host "     ⚠️  Failed to configure auto-shutdown" -ForegroundColor Yellow
                Write-Host "     Error: $($_.Exception.Message)" -ForegroundColor DarkGray
            }
...
function Generate-CriblCollectors {
    ...
    if ($null -eq $StorageAccount) {
        Write-Host "`n  ℹ️  Storage Account not available - skipping Cribl collector generation" -ForegroundColor Cyan
        return
    }
    ...
    if (-not $SkipWait) {
        Write-Host "`n  ⚠️  NOTICE: Flow logs typically take 5-10 minutes to start after VMs begin generating traffic" -ForegroundColor Yellow
        ...
        if (-not $containerExists) {
            if ($userSkipped) {
                Write-Host "`n  ⏭️  Skipped waiting for flow logs" -ForegroundColor Yellow
            }
            Write-Host "  ⚠️  Flow log container not created yet - this is normal if VMs just started" -ForegroundColor Yellow
        ...
        try {
            ...
            if ($null -ne $container) {
                ...
                Write-Host "  ⚠️  Flow log container not found - flow logs may not have started yet" -ForegroundColor Yellow
                Write-Host "     Wait 5-10 minutes after VMs start generating traffic, then try again" -ForegroundColor Gray
            }
        } catch {
            Write-Host "  ⚠️  Could not access storage container: $($_.Exception.Message)" -ForegroundColor Yellow
        }
    }
    ...
    if ($containerExists) {
        try {
            ...
            if ($flowLogPaths.Count -gt 0) {
                ...
                Write-Host "  ⚠️  No flow log paths discovered yet" -ForegroundColor Yellow
            ...
            Write-Host "  ⚠️  Could not discover flow log paths: $($_.Exception.Message)" -ForegroundColor Yellow
        }
    }
    ...
    if ($primaryFlowLogPath -eq "/<FLOW_LOG_RESOURCE_PATH_TBD>") {
        Write-Host "`n  ⚠️  Flow log paths not yet available - collectors contain placeholder paths" -ForegroundColor Yellow
